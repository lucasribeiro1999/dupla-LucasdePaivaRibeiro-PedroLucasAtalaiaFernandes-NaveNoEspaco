/* ======================================================================
   ...
   Polimento Visual (Item 6) incluído:
   - Paralaxe mais profunda (4 camadas + poeira/nébula)
   - Offsets distintos (drift X/Y por camada)
   - Motion blur falso (trailing da nave)
   - Tipografia do HUD (sombra no texto do canvas)
   - Feedback de colisão (flash de tela + knockback leve)
====================================================================== */
...
(() => {
  ...
  class Player {
    ...
    update(dt){
      ...
      // trailing (salva posição a cada ~2 frames)
      this._trailAcc += dt;
    ...
    draw(ctx){
      // desenha trailing (motion blur falso)
      for(let i=this.trail.length-1;i>=0;i--){
        const t=this.trail[i];
        const alpha = (i+1)/(this.TRAIL_MAX*6); // bem sutil
    ...
    hit(){
      ...
      // knockback para longe do centro (rápido e simples)
  ...
  const starsUltraFar = makeStars(70, 10,  -4);  // bem distante, drift para a esquerda
  const starsFar      = makeStars(60, 20,   6);
  const starsMid      = makeStars(50, 40,  -12);
  const starsNear     = makeStars(40, 80,   18);

  // poeira nebulosa (círculos grandes e suaves)
  const nebula = new Array(6).fill(0).map(()=>({
    x: Math.random()*WIDTH, y: Math.random()*HEIGHT, r: 80+Math.random()*120, sy: 6+Math.random()*6, sx: (Math.random()<.5?-1:1)*3
  }));
  ...
  function draw(){
    ...
    drawNebula(); // abaixo das estrelas médias para um glow bonito
    drawStars(starsFar);
    drawStars(starsMid);
    drawStars(starsNear);
