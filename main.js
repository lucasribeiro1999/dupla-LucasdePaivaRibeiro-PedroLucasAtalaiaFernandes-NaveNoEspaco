/* ======================================================================
   ...
   + Música de fundo (MP3) em loop:
     - arquivo em /audio/music.mp3
     - se aberto via file:// usa <audio> (fallback)
     - se aberto via http/https usa Web Audio API (loop mais suave)

   + Cronômetro no HUD
   + Recorde salvo em localStorage
   + Tela de Game Over mostra Score, Recorde e "NOVO RECORDE!"
====================================================================== */
...
(() => {
  ...
  // ===== Cronômetro + HighScore =====
  const HS_KEY = 'space_patrol_highscore_v1';
  let elapsed = 0;
  let highScore = Number(localStorage.getItem(HS_KEY) || 0);
  let newRecord = false;

  // ====== Áudio (Web Audio API + fallback <audio>) ======
  const htmlAudio = document.getElementById('bgm');
  htmlAudio.volume = 0.35;

  // Se aberto como file://, fetch/AudioContext podem falhar → usar fallback.
  const USE_FALLBACK = location.protocol === 'file:';
  ...
  if (AudioMgr) {
    ...
    audioReady = AudioMgr.load('bgm', 'audio/music.mp3');
  }
  ...
  async function startMusic() {
    ...
    try {
      if (AudioMgr) {
        if (AudioMgr.Ctx.state !== 'running') await AudioMgr.Ctx.resume();
        await audioReady;
        bgmHandle = AudioMgr.playLoop('bgm', { volume: 0.35 });
        if (bgmHandle) return;
      }
  ...
  const asteroids = []; const resetAsteroids = () => { asteroids.length = 0; };
  ...
  function draw() {
    ...
    if (gameState === GameState.MENU) {
      drawTitle('NAVE NO ESPAÇO', 'Pressione ENTER ou clique em Jogar'); return;
    }
    if (gameState === GameState.GAMEOVER) {
      drawHUD();
      drawGameOverOverlay(); return;
    }
  ...
  function drawHUD() {
    ...
    ctx.fillText(`Tempo: ${fmtTime(elapsed)}`, 16, 66);
    ctx.restore();
  }
  ...
  function drawGameOverOverlay() {
    ...
    centerText('GAME OVER', HEIGHT / 2 - 40);

    ctx.fillStyle = '#e6f1ff';
    ctx.font = '20px system-ui,Segoe UI,Roboto,sans-serif';
    const lines = [
      `Pontuação: ${score}`,
      `Recorde: ${highScore}` + (newRecord ? '  —  NOVO RECORDE! 🎉' : ''),
      `Tempo: ${fmtTime(elapsed)}`,
      'Pressione ENTER para reiniciar'
    ];
    let y = HEIGHT / 2 + 8;
    for (const line of lines) { centerText(line, y); y += 26; }
  }
